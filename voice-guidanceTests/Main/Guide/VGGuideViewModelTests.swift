...
class VGGuideViewModelTests: XCTestCase {
  ...
  override func setUpWithError() throws {
    let storageProvider = VGStorageProvider(source: Bundle(for: type(of: VGGuideViewModelTests())), name: "VGCoreData", group: "group.org.kenzan8000.voice-guidanceTests")
  ...
  func testVGGuideViewModel_whenTtsStartedSpeaking_ttsShouldBeSpeaking() throws {
    ...
    sut.playTts()
    DispatchQueue.main.asyncAfter(deadline: .now() + 7.5) { exp.fulfill() }
